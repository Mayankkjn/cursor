...
function renderAutomationOpportunities() {
  ...
  shown.forEach((item, i) => {
    ...
    footer.append('button').attr('class', 'task-detail-footer-btn primary').attr('type', 'button').text('Automate with Seek →')
      .on('click', () => openAutomationPreview(node.id));
  });
...
const FLOW_ARROW_SVG = '<svg width="12" height="16" viewBox="0 0 12 16" fill="none" aria-hidden="true"><path d="M6 0 V11 M2 7 L6 12 L10 7" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg>';

// The "bridge" between a task's automation score and the (inert) Seek
// handoff: a preview of exactly what would be automated and what Seek
// would receive, built entirely from the same real signals as the rest of
// the Automation tab — coverage is consistency (1 - deviation rate), the
// step list is the task's own real subtypes when the dataset has them,
// and nothing is invented when it doesn't.
let automationPreviewState = null;

function openAutomationPreview(taskId) {
  const model = state.model;
  const node = model.nodes.find((n) => n.id === taskId);
  if (!node) return;
  const meta = state.taskInsights && state.taskInsights.byTaskName.get(taskId);
  ...
  const coverage = m.consistency;
  const expectedAutomated = Math.round(node.visits * coverage);
  const exceptionsPct = 1 - coverage;
  const timeSaved = coverage * node.totalTime;
  const steps = meta && meta.subtypes && meta.subtypes.length ? meta.subtypes.map((s) => s.name) : null;

  automationPreviewState = { node, meta, coverage, expectedAutomated, exceptionsPct, timeSaved, steps };

  d3.select('#preview-task-title').text(node.label);
  const foundDominant = coverage >= 0.5;
  d3.select('#preview-lens-found').text(
    foundDominant ? 'Lens found 1 dominant execution pattern' : 'Lens found no single dominant execution pattern'
  );
  d3.select('#preview-lens-found-sub').text(
    foundDominant
      ? `${pct(coverage)} of executions follow this exact sequence`
      : 'Executions are split across multiple paths — automation coverage would be lower'
  );

  const flow = d3.select('#preview-flow-diagram');
  flow.selectAll('*').remove();
  if (steps) {
    steps.forEach((name, i) => {
      if (i > 0) flow.append('div').attr('class', 'flow-arrow').html(FLOW_ARROW_SVG);
      flow.append('div').attr('class', 'flow-step').text(name);
    });
  } else {
    flow.append('p').attr('class', 'task-detail-muted-note').text('Not available — no step-level breakdown in this dataset.');
  }

  d3.select('#preview-recommendation-text').text(
    m.tier === 'low'
      ? 'Automation coverage is limited for this task — consider a partial or supervised approach instead.'
      : 'Automate the primary path and route exceptions to a human.'
  );

  d3.select('#preview-stat-coverage').text(`${pct(coverage)} of executions`);
  d3.select('#preview-stat-expected').text(`${expectedAutomated.toLocaleString()} / ${node.visits.toLocaleString()} executions`);
  d3.select('#preview-stat-time-saved').text(`${formatDuration(timeSaved)} (total)`);

  d3.select('#automation-preview-step-1').classed('hidden', false);
  d3.select('#automation-preview-step-2').classed('hidden', true);
  d3.select('#automation-preview-modal').classed('hidden', false);
}

function closeAutomationPreview() {
  d3.select('#automation-preview-modal').classed('hidden', true);
}

// Renders the exact "context package" Seek would receive — no live
// integration exists, so this is a transparent preview of the handoff
// rather than a real API call.
function buildSeekPayloadText() {
  const { node, meta, coverage, exceptionsPct, steps } = automationPreviewState;
  const pct = (n) => `${Math.round(n * 100)}%`;
  const lines = [];
  lines.push('Automation request generated by Lens');
  lines.push(`Automate ${node.label}`);
  lines.push(`Primary path: ${pct(coverage)} of executions`);
  lines.push('');
  lines.push(`Applications: ${meta && meta.appId ? meta.appId : 'Not tracked in this dataset'}`);
  lines.push('');
  if (steps) {
    lines.push('Steps:');
    steps.forEach((s, i) => lines.push(`${i + 1}. ${s}`));
  } else {
    lines.push('Steps: not available — no step-level breakdown in this dataset.');
  }
  lines.push('');
  lines.push(`Exceptions: ${pct(exceptionsPct)} of executions require manual review`);
  lines.push(`Expected coverage: ${pct(coverage)}`);
  return lines.join('\n');
}

d3.select('#automation-preview-send').on('click', () => {
  d3.select('#preview-payload').text(buildSeekPayloadText());
  d3.select('#automation-preview-step-1').classed('hidden', true);
  d3.select('#automation-preview-step-2').classed('hidden', false);
});
d3.select('#automation-preview-close').on('click', closeAutomationPreview);
d3.select('#automation-preview-close-2').on('click', closeAutomationPreview);
d3.select('#automation-preview-modal').on('click', function (event) {
  if (event.target === this) closeAutomationPreview();
});
...
d3.select('#task-detail-automate-seek').on('click', () => {
  if (state.selectedTaskId) openAutomationPreview(state.selectedTaskId);
});
...
d3.select(document).on('keydown.sessionReplay', (event) => {
  ...
  if (!d3.select('#automation-preview-modal').classed('hidden')) closeAutomationPreview();
  else if (!d3.select('#session-replay-modal').classed('hidden')) closeSessionReplay();
