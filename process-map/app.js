...
const state = {
  ...
  expandedBubbles: new Set(), // source node ids whose "+N minor variants" bubble is currently expanded
};
...
// "+N minor variants" bubble instead of each getting its own permanent
...
function buildRenderGraph(model) {
  ...
  const survivingEdges = model.edges.filter(edgeSurvives);

  // How many surviving edges touch each node. A minor branch is only safe
  // to collapse behind a bubble if hiding its destination can't orphan some
  // other edge that relies on that node still being drawn — either the
  // destination is a pure dead end (this is its only connection), or it's a
  // one-step detour that terminates the case (its only other edge goes to
  // END, which stays visible regardless via other paths).
  ...
  // Decide, per fork, which of its edges bundle into a bubble.
  ...
  deviationsByFrom.forEach((group, from) => {
    ...
    group.forEach((e) => {
      if (e.caseCount / total >= MINOR_SHARE) return;
...
// minor variants — click to expand"; once expanded it flips to a
// "− Collapse" toggle in the exact same spot, so the affordance to put the
// clutter away again is always right where the bubble was.
function buildBubble(g, n, p) {
  const label = n.expanded
    ? `− Collapse ${n.count} variant${n.count === 1 ? '' : 's'}`
    : `+${n.count} minor variant${n.count === 1 ? '' : 's'}`;
...
function nodeTooltipHtml(n, model) {
  ...
  if (n.kind === 'bubble') {
    ...
    return `<strong>${n.count} minor variant${n.count === 1 ? '' : 's'}</strong>${rows}<div>${n.expanded ? 'Click to collapse' : 'Click to expand'}</div>`;
  }
...
function edgeTooltipHtml(e) {
  ...
  if (e.isBubbleEdge) {
    return `<strong>${e.sourceEdges.length} minor variants from ${labelForNode(e.from)}</strong><div>${pluralCases(Number(e.label))} total</div>`;
  }
